...
public class HappyValentinsDay extends JFrame {
  ...
  private void setDialog(JDialog diag, String tittle, String txt) {
    ...
    diagBut.setBounds(155, 150, 100, 30);
    ImageIcon img = new ImageIcon("src/main/resources/logo.jpg");
    img.setImage(img.getImage().getScaledInstance(100,60, Image.SCALE_DEFAULT));
    JLabel temp = new JLabel(img,SwingConstants.CENTER);
    temp.setBounds(155, 70, 100, 60);
    ...
    diag.setBounds(screenWidth/2-225, screenHeight/2-100, 450, 250);
    diag.setLayout(null);
    diag.add(diagBut);
    diag.add(temp);
    diag.add(diagLabel);
    diag.setVisible(true);
    new showMessageFrame("momo");
    new showMessageFrame("momo",300,200);
    new showMessageFrame("momo",-300,-200);
  }// setDialog
  /**
   * 设置单击窗口关闭按钮时的动作
